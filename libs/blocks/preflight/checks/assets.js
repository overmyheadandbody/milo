...
function getAssetData(asset) {
  ...
  if (type === 'mpc' && asset.closest('.background')) {
    notes = 'Used as background, Sharepoint should be used instead.';
  }
...
function populateAssetMeta(asset, assetData) {
  ...
  const assetMetaParent = (assetData.type === 'video' && '.video-holder')
    || (assetData.type === 'mpc' && '.milo-video')
    || 'picture';
  ...
  const assetMessage = createTag(
    'div',
    { class: `asset-meta-size preflight-decoration ${assetData.hasMismatch ? 'has-mismatch' : 'no-mismatch'}` },
